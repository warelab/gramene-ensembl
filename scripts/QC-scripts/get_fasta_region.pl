...
get_fasta_region.pl - get the sequence from the specified sequence region in the fasta files                      This is usually used for QC check when the ensembl translated peptides 
                     differ from downloaded peptides and a manuall inspection is need.
...
or die "cannot create write Bio::SeqIO";
...
sub search_fasta{
  ...
  while( my $seq=$fasta_seqio->next_seq()){
    ...
    if( $seq_name eq $header ){
      ...
      $out->write_seq(
		      Bio::Seq->new(-display_id => ">$header:$start-$end",
				 -seq => $out_seq)

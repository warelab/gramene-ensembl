...
# it won't preserve the database IDs 
...
my ($_help, $_version, $sdbhost, $sdbport, $sdbuser, $sdbpass, $sdbname, 
    ...
    $glen, $plen, $write, $usechr, $baselen, $logic_name, $lfile);
GetOptions(
	   'h|help'        => \$_help,
	   'v|version'     => \$_version,
	   'dbhost=s'      => \$sdbhost,
...
my $genes;
my $transcripts;
my $sgene_adaptor =  $sdba->get_GeneAdaptor;
#my @gene_stable_ids;

if ($lfile){
    open my $fh, $lfile or die "Cannot open gene stable id file for reading";
    push @$genes, map{ print; chomp;$sgene_adaptor->fetch_by_stable_id($_) } <$fh>;
    close $fh;
}

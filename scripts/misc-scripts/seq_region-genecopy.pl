...
$genes = get_all_genes($sdba, $type, $coord_system, $sr_name);
...
sub get_all_genes {
    my ($dba, $type, $coord_system, $sr_name) = @_;
    my $slice_adaptor = $dba->get_SliceAdaptor;
    my @genes;
    my $sr_slice = $slice_adaptor->fetch_by_region($coord_system, $sr_name);
    print STDERR  " $coord_system(s)\n";
    for my $slice ($sr_slice) {

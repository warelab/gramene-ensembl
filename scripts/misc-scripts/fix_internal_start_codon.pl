  ...
  foreach my $trans (@transcripts) {
    ...
    if($aa =~ /^M/i){
		...
		if( $aa =~ /M/i){
	    	...
	    	print "$update_exon_start_sql with [$start_exon_start_phase, $met_start_ExonID]" if $debug;	
	    	print "$update_translation_sql for $met_start_ExonID, $start_exon_start, $translation_id" if $debug;

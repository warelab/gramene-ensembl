...
#    $tax =~ s/ /_/g;
    ...
    $tax = $species if $tax eq 'Zea mays';
    if (!$tax){
	$tax = $sp_convert{$species} || $species;
    }
...
my @taxa = sort { $colors{$a} cmp $colors{$b} } keys %taxa;
...
sub taxon_convert {
    my %tax_convert = (
	'BEP_clade' => 'B\EP_clade',  #this is how it is written accidentially in the iTOL tree "NAM_grant";

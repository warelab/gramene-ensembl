# Ensembl module for Bio::EnsEMBL::Analysis::Runnable::SyntenyBlock
#
...
Bio::EnsEMBL::Analysis::Runnable::SyntenyBlock
    
=head1 SYNOPSIS

  my $synteny_block = Bio::EnsEMBL::Analysis::Runnable::SyntenyBlock->
  new(
    -program => 'synteny_block.pl',
    -ort => $ort, #ort data in arrayref
	-query_species => "rice", 
	-target_species => "sorghum", 
    );
  $synteny_block->run;
  my @blocks = @{$synteny_block->output};

=head1 DESCRIPTION

Runs the program SyntenyBlock.pl and produces synteny blocks 
...
package Bio::EnsEMBL::Analysis::Runnable::SyntenyBlock;
...
sub new {
	...
	#SETTING DEFAULTS#
	$self->program('synteny_block.pl') if (!$self->program);

	my ($ort, $query_species, $target_species) = rearrange([ 'ORT', 'QUERY_SPECIES', 'TARGET_SPECIES' ], @args);
...
sub ort{
	...
	my $ort=shift;
	if ($ort) {
		throw("[*DIE] Must pass Runnable::SyntenyBlock::ort an arrayref not a ".
...
=head2 ortfile
  ...
  Arg [1]   : Bio::EnsEMBL::Analysis::Runnable::SyntenyBlock
...
sub ortfile{
  ...
  if(!$self->{'ortfile'}){
	my $name=join "_", map { join '', map {substr $_, 0, 1} split /_|\s/, $self->{$_} } ('query_species', 'target_species');
    $self->{'ortfile'} = $self->create_filename($name, 'ort');
  }
  if(!$self->resultsfile){
	my $resultsfile = $self->{'ortfile'};
	$resultsfile=~s/\.ort$/.blocks/;
...
sub write_ortfile{
	my ($ort, $filename)=@_;
	if (!$ort || !$filename) {
		throw("[*DIE] Must pass Runnable::SyntenyBlock::ort an arrayref of ortholog data and a filename");
	} else {
		throw("[*DIE] Must pass Runnable::SyntenyBlock::ort an arrayref not a ".
			$ort) unless (ref($ort) eq 'ARRAY');
...
sub run_analysis {
    ...
    my $cmd = $self->program . " " . $self->ortfile;

    print "Running analysis " . $cmd . "\n";
	system "cp", $self->ortfile, "$ENV{HOME}";
    system($cmd) == 0
        or throw("[*DIE] FAILED to run " . $cmd . " SyntenyBlock.pm:run_analysis");

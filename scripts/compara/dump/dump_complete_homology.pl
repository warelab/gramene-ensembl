...
BEGIN{  #Argument Processing
  ...
  my $out_file_homo = "homology_report";
...
$sqls{homology_pairs} = qq(select hm1.seq_member_id, hm2.seq_member_id, h.homology_id,
                g1.name, g2.name, sm1.stable_id, sm2.stable_id, h.description,
                h.wga_coverage, h.is_tree_compliant, h.is_high_confidence,
                hm1.perc_id, hm1.perc_cov, hm2.perc_id, hm2.perc_cov
                ...
                hm1.seq_member_id < hm2.seq_member_id);  #253467 rid example
...
MAIN:{
  ...
  foreach my $rid( keys %genetree_uniq_seqids ){
	...
	my %homology_seqid_pairs = &get_homology_pairs_by_rid( $rid );
    	
	warn("[INFO] number of seqid pairs is ", scalar @{$seqid_pairs}, "\n");
	warn("[INFO] number of homology is ", scalar keys %homology_seqid_pairs, "\n");
	my @missing_pairs_in_homology = grep { !$homology_seqid_pairs{$_} } @{$seqid_pairs};
	if ($TEST){
		map{ warn "inspect:$_\n"; warn "Not in homology:$_\n" if !$homology_seqid_pairs{$_} } @{$seqid_pairs};
		map{ warn "homolgy pair:$_\n" } keys %homology_seqid_pairs;
	}
	my %seqid_pairs;
	map{ $seqid_pairs{$_} = 1 } @{$seqid_pairs};
	my @reverse_missing_pairs = grep { !$seqid_pairs{$_}  } keys %homology_seqid_pairs; 
	warn "[INFO] the number of pairs missing from homology table is " . scalar @missing_pairs_in_homology . "\n";
	warn "[INFO] the number of pairs excessive from homology table (should always be 0) is " . scalar @reverse_missing_pairs . "\n";
  
	die "There are extra pairs in homology that's not in the tree" 
		if scalar @reverse_missing_pairs > 0;

	warn "[INFO] report homologies \n"; 
	map{ print $FH_HOMO join "\t", @{$homology_seqid_pairs{$_}{report}}; print $FH_HOMO "\t$rid\n" } 
				keys %homology_seqid_pairs;

	warn "[INFO] report up to 1000 missing paris that have paf.evalue < 0.01 and paf.score > 100\n";
	&report_missing_pairs(\@missing_pairs_in_homology, $rid);
...
sub get_homology_pairs_by_rid{
  ...
  my $sql = qq(select hm1.seq_member_id, hm2.seq_member_id, h.homology_id,
		g1.name, g2.name, sm1.stable_id, sm2.stable_id, h.description,
		h.wga_coverage, h.is_tree_compliant, h.is_high_confidence,
		hm1.perc_id, hm1.perc_cov, hm2.perc_id, hm2.perc_cov 
		from homology_member hm1 join homology_member hm2 
  ...
  my $rv = $sths{homology_pairs}->execute( $rid) || die( $sths{homology_pairs}->errstr );
  my %homology;
  while( my $row = $sths{homology_pairs}->fetchrow_arrayref ){
	...
	$homology{$seq_id_pair}{report} = [
		$row->[3] || 'NA', $row->[5] || 'NA', $row->[4] || 'NA', 
		$row->[6] || 'NA', $row->[7] || 'NA', 
		$row->[8] || 'NA', $row->[9] || 'NA', $row->[11] || 'NA', $row->[12] || 'NA', 
		$row->[13] || 'NA', $row->[14] || 'NA', $row->[10] || 'NA'
		];

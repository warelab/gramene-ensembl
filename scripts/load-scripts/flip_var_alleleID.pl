...
my $allele_adaptor = $dba->get_AlleleAdaptor( $species, 'variation_feature' );
my $sth_aa = $allele_adaptor->prepare("select max(allele_id) from allele");
$sth_aa->execute; 
my ($max_alleleid) = $sth_aa->fetchrow_array();
print "the max allele ID is $max_alleleid\n";
$sth_aa->finish;

my $variationfeature_adaptor = $dba->get_VariationFeatureAdaptor( $species, 'variation_feature' );
my $sth_vf = $variationfeature_adaptor->prepare("select variation_id from variation_feature where variation_feature_id=?");
...
#	print "key=$vfid, $vfid2allelestr{$vfid}\n";
	...
	$sth_vf->execute($vfid);
	my ($vid) = $sth_vf->fetchrow_array();
	push @vids, $vid;
} 

$sth_vf->finish;

my $tmp_aid = $max_alleleid + 10;
my $sth_aavid = $allele_adaptor->prepare("select allele_id from allele where variation_id=?");
my $sth_aa_swap = $allele_adaptor->prepare("update allele set allele_id=? where allele_id=?");

for my $avid( @vids ){
	$sth_aavid->execute( $avid );
	my $rows = $sth_aavid->fetchall_arrayref();
	my @aids = map{ $_->[0] } @{$rows};

	my $allele_cnt = scalar @aids;
	if ($allele_cnt != 2 ){
		warn("Warn, there are $allele_cnt allele for this variation_id $avid, expecting 2, skip");
		next;
	}
	#map { print "$_\n" } @aids;

	#now swap allele_id of the two alleles
	$sth_aa_swap->execute( $tmp_aid, $aids[0]);
	$sth_aa_swap->execute( $aids[0], $aids[1]);
	$sth_aa_swap->execute( $aids[1], $tmp_aid);

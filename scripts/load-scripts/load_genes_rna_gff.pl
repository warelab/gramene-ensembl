...
load_genes_rna_gff.pl - load tRNA rRNA genes into EnsemblDB

=head1 SYNOPSIS

  load_genes_rna_gff.pl [options] gff_file
...
=head1 DESCRIPTION
  ...
  Parses tRNA. rRNA annotations from AGI gff file , and uses the data
...
Readonly my $RNA_REGEX => qr/^(RRNA_FRAGMENT|TRNA)$/i;
...
foreach my $geneid( sort keys %$GENES ){
  ...
  $eGene->description( $genedata->{GENE_DESCRIPTION} || $genedata->{GENE_NAME_ALIAS} || '');
  $eGene->version(1);  
  ...
  my $gene_xref = [{
		    ...
		    description          => $genedata->{GENE_DESCRIPTION}   || $genedata->{GENE_NAME_ALIAS} || '',
		   },
                  ];
  ...
  foreach my $trptid( sort keys %{$genedata->{TRPT}} ){
    ...
    $eTranscript->description( $trptdata->{TRPT_DESCRIPTION} || $trptdata->{TRPT_NAME_ALIAS} || '');
    $eTranscript->version( 1 );    
    $eTranscript->biotype( $trptdata->{BIOTYPE} );
    ...
    my $transcript_xref = [{
			    ...
			    description   => $trptdata->{TRPT_DESCRIPTION}  || $trptdata->{TRPT_NAME_ALIAS} || '',
			   },
			  ];

...
load_genes_from_jgi_gff3.pl - parse genes out from JGI gff3 file and load them into EnsemblDB
                              use Poplar v2.0 gff3 file as example

=head1 SYNOPSIS

  load_genes_from_jgi_gff3.pl [options] gff_file
...
Readonly my @NAME_FIELDS => qw(NAME ALIAS ID);
...
use vars qw( $ENS_DBA $I $INSERT $GFF_HANDLE $ANALYSIS $XREF_SRCS );
...
while( my $line = $GFF_HANDLE->getline ){
  ...
  if( $feature eq 'GENE' ){
    ...
    $gene_id   = $attribs{'ID'};
    $gene_name = get_first_func(\%attribs, @NAME_FIELDS); 
    ...
    $transcript_name = get_first_func->(\%attribs, @NAME_FIELDS);
...
#
GENE:foreach my $g( keys %$GENES ){
  my $genedata = $GENES->{$g};
  $genedata->{TRANSCRIPT_COUNT} = keys %{$genedata->{TRANSCRIPTS}};
    
  
TRPT:  foreach my $t( keys %{$genedata->{TRANSCRIPTS}} ){
...
sub compute_exons{ 
  ...
  for (my $i=1; $i<$start_cnt; $i++){
	...
	$trpt_ex_starts->[$i] >= $trpt_ex_starts->[$i-1]){

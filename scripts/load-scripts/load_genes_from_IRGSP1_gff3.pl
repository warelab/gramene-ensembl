...
Readonly my $DESCRIPTION => 'DESCRIPTION';
...
while( my $line = $GFF_HANDLE->getline ){
  ...
  my $description = $attribs{$DESCRIPTION} if defined $attribs{$DESCRIPTION} ;
...
print "\n\n[DEBUG] get_gene_id $gene_id, feature=$feature xxxx\n\n" if $V;
  if( $feature =~ $GENE_REGEX){
    ...
    $GENES->{$gene_id}->{STRAND}     = $strand eq '+' ? 1 : -1;
     $GENES->{$gene_id}->{DESCRIPTION} = $description if $description;  
  }
  ...
  if( $feature eq 'MRNA' ){
    ...
    $GENES->{$gene_id}->{TRPT}->{$id}->{DESCRIPTION} = $attribs{NOTE} if $attribs{NOTE};
...
foreach my $geneid( sort keys %$GENES ){
  ...
  foreach my $trptid( sort keys %{$genedata->{TRPT}} ){
    ...
    #print "[debug] trpt_name = $trpt_name\n";
   ...
   # print "exon count = $trptdata->{EXON_COUNT}\n";

...
#select feature_stable_id from transcript_variation limit 5;

use vars qw( $ENS_DBA $dbh $I $INSERT $F_HANDLE  $IGNORE $V $STH );
our $sql_update_sid = qq(
                update transcript_variation set feature_stable_id=? where feature_stable_id=?
        );
my $date = time(); 
  ...
  $species || pod2usage("Need species\n");

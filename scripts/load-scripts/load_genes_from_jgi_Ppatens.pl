...
while( my $line = $GFF_HANDLE->getline ){
  ...
  my $id = $attribs{ID} || $name;
  
  if ( defined $attribs{PARENT} ){
      $PARENT{$id} = $attribs{PARENT};
  }

  my $gene_id = get_gene_id($id);
  

print "\n\n[DEBUG] get_gene_id $gene_id, feature=$feature xxxx\n\n";
  if(  !$GENES->{$gene_id} ){
    print "[INFO] Processing gene $gene_id" if $V;
    $GENES->{$gene_id}->{GENE_NAME}  = $name;
  ...
  if( $feature eq 'MRNA' && !$GENES->{$gene_id}->{TRPT}->{$id} ){
    
    #$GENES->{$gene_id}->{TRPT}->{TRPT_COUNT}++;
    $GENES->{$gene_id}->{TRPT}->{$id}->{TRPT_NAME}  = $name; 
    $GENES->{$gene_id}->{TRPT}->{$id}->{EXON_COUNT} = 0;
    $GENES->{$gene_id}->{TRPT}->{$id}->{CDS_COUNT}  = 0;
    $GENES->{$gene_id}->{TRPT}->{$id}{STRAND}     = $strand eq '+' ? 1 : -1;
  }
  ...
  if( $feature eq 'EXON' #||
    ...
    my $trpt_id = $attribs{PARENT} || $id;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{EXON_COUNT} ++;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{EXON}{$start} = $end;
  }

  if( $feature =~ $CDS_REGEX ){
      my $trpt_id = $attribs{PARENT} || $id;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{CDS_COUNT} ++;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{CDS_EXON}{$start} = $end;
  }

  if( $feature eq 'START_CODON' ){
      my $trpt_id = $attribs{PARENT} || $id;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{START_CODON} = 
      $strand eq "+" ? $start : $end;
  }
  
  if( $feature eq 'STOP_CODON' ){
      my $trpt_id = $attribs{PARENT} || $id;
    $GENES->{$gene_id}->{TRPT}->{$trpt_id}->{STOP_CODON} = 
      $strand eq "+" ? $end : $start;
  }
...
foreach my $geneid( sort keys %$GENES ){
  ...
  foreach my $dbentry( make_dbentries($gene_xref) ){
    ...
    if( $dbentry->dbname eq 'JGI_GENE' ){ # Use as display_xref
  ...
  foreach my $trptid( sort keys %{$genedata->{TRPT}} ){
    ...
    foreach my $dbentry( make_dbentries($transcript_xref) ){
      ...
      if( $dbentry->dbname eq 'JGI_TRANSCRIPT' ){ # Use as display_xref

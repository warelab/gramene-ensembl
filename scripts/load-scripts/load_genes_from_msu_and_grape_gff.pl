...
while( my $line = $GFF_HANDLE->getline ){
  ...
  my ($gene_id, $gene_name, $gene_description, $transcript_id, $transcript_name, $parent_id);

  if( $feature eq 'GENE' ){

    $gene_name = $attribs{'ALIAS'};
    $gene_id   = $attribs{'ID'} ? $attribs{'ID'} : $gene_name;
    $gene_description = $attribs{'GENE'};
    
    print "Gene $gene $gene_description\n";#####
    unless( $GENES->{$gene} ){
    ...
    if( $gene_name = $attribs{'GENE'} ){ #format in Grape gff
      unless( $GENES->{$gene_name} ){
	$GENES->{$gene_name}->{GENE_NAME} = $gene_name;
	$GENES->{$gene_name}->{SEQ_NAME}  = $seqname;
	$GENES->{$gene_name}->{START} = $start; #always smaller than end
	$GENES->{$gene_name}->{END}  = $end;
	$GENES->{$gene_name}->{STRAND}     = $strand eq '+' ? 1 : -1;
      }
      
    
      $transcript_name = $attribs{'MRNA'};
      unless( $GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript} ){
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{TRPT_NAME} = $transcript_name;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{SEQ_NAME} = $seqname;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{START} = $start; 
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{END}  = $end;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{EXON_COUNT} = 0;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{CDS_COUNT} = 0;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{UTR_COUNT} = 0;
	$GENES->{$gene_name}->{TRANSCRIPTS}->{$transcript_name}->{STRAND} = 
	  $strand eq '+' ? 1 : -1;
      }
      
      #establish the mapping from transcript to gene
      $TRPT2GENE->{$transcript_name} = $gene_name;
      print"mRNA $transcript_name => $gene_name, $TRPT2GENE->{$transcript_name}\n";#####
    }elsif( $transcript_id = $attribs{ID} ){
      $transcript_name = $attribs{ALIAS};
      $parent_id = $attribs{PARENT};
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{TRPT_NAME} = $transcript_name;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{SEQ_NAME} = $seqname;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{START} = $start; 
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{END}  = $end;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{EXON_COUNT} = 0;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{CDS_COUNT} = 0;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{UTR_COUNT} = 0;
      $GENES->{$parent_id}->{TRANSCRIPTS}->{$transcript_name}->{STRAND} = 
	$strand eq '+' ? 1 : -1;
    }

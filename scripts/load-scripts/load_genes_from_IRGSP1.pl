...
#  next;
  ...
  $feature = uc($feature);
  $seqname =~ s/Bd//i;
  #print "$seqname, $source, $feature, $start, $end, $score, $strand, $frame, $attribute\n";
  my %attribs;
 ...
 #     print "attrib mapping $1 => $2\n";
  ...
  my $gene_id = $id ? $id : $parent;
 if( $feature ne 'GENE' ){
      my @p = split '-', $gene_id;
      $gene_id = $p[0];
      $gene_id =~ s/T/G/;
#      $PARENT{$id} = $feature eq 'MRNA' ? $gene_id:$parent;
   }
...
foreach my $geneid( sort keys %$GENES ){
  ...
  print "geneid=$geneid, seq=", $genedata->{SEQ_NAME}, "\n";
...
print "trpid=$trptid\n";
    ...
    # EXONS #
    #########
    $trptdata->{EXON_COUNT} =  scalar @{$trptdata->{EXON_START}};

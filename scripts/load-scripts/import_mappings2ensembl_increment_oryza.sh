...
perl $SCRIPT -registry_file $REG -species $SP -mapset_name $MAPSET -marker_type TE-T-DNA-INSERTION  -track_species ${TRACK_SP} -date $DATE -cs toplevel

perl $SCRIPT -registry_file $REG -species $SP -mapset_name $MAPSET -marker_type chemical-mutations  -track_species ${TRACK_SP} -date $DATE -cs toplevel

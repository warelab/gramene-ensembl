...
B<-not_seq_level --not_seq_level>
  Not sequence level coordinate system
...
sub store_taxon_meta{
  ...
  my $species_node;
  my @taxon_nodes = split /\s+|_/, $taxon->scientific_name;
  unless ($taxon_nodes[1]){
    print "ERR: Wrong species name, ", $taxon->scientific_name,
    ", need something like 'Oryza_sativa'\n";
    return ;
  }
  $species_node = Bio::Taxon->new(-name      => $taxon_nodes[1],
				#  -rank      => $rank,
                                  );
  ...
  foreach my $node( $species_node, reverse $tree_functions->get_lineage_nodes($taxon) ){
    #warn( Dumper ($node));
...
sub get_coord_systems{
  ...
  if($seq_cs){
    ...
    $seq_cs = Bio::EnsEMBL::CoordSystem->new
      (
       -name    => $CS_NAME,
       -rank    => $RANK ? $RANK : $CS_NAME =~ /chromosome/i ? 1 : 2,
       -default => 1,
       -version => $VERSION, 
       $CHUNK_SIZE ? () : 

...
sub main {
	...
	while(<$in_file_handle>) {
		...
		# data
			...
			#add loc_id for rsID search by loc_id
			my $loc_id = "$split[0]:$split[1]:$split[3]_$split[4]"; 
			#warn("DEBUG loc_id=$loc_id\n");
			my $rsid_arrayref = get_rsid( $config, $loc_id);	
			#push @{$data->{synonyms}}, $rsid if defined $rsid_arrayref;
			...
			#$data->{tmp_vf}->{variation_name} = $rsid if $rsid;
	...
	$chromosome =~ s/$prefix//i;
      ...
      if( defined $rsid_arrayref ){
	   if ( $data->{ID} eq $rsid_arrayref->[0]){
		$config->{skipped}->{identical_rsids}++;
		$last_skipped++;
	   }

	   push @{$data->{synonyms}}, $data->{ID};
	   $data->{ID} = $rsid_arrayref->[0];
      } 
	  ...
	  if(length($data->{ALT}) > 50000 || length($data->{REF}) > 50000) {
		...
		next;
			...
			$data->{variation} = defined $rsid_arrayref ?
		       				$rsid_arrayref->[1] :
						variation($config, $data);
		
			#warn("DEBUG v name = ", $data->{ID}, ", vf name is either $rsid or ", $data->{ID}, "\n"); 
		 #$data->{tmp_vf}->{variation_name} = $data->{rsid} ? $data->{rsid} : $data->{ID};
...
# get variation object with the same ID, if there is none, we need to create a new variation obj but 
# we dont want variation_feature name to be rsID if there is already an rsID for this var. 
# Due to variation table unique key constraint for name, we cannot use rsID for the name of var in this table
sub get_rsid{
	my $config = shift;
	my $loc_id   = shift;

	my $var_features = $config->{variationfeature_adaptor}->fetch_all_by_location_identifier($loc_id);

	foreach my $vf( @{$var_features} ){
		#warn("DEBUG in var_features\n");
	        my $vid  = $vf->variation_name();
		#warn("DEBUG vn = $vid\n");
	     if ( $vid =~ /^rs\d+/i){
		     #warn(" DEBUG Found rsID $vid\n");
		return [$vid, $vf->variation];
     	     }	
       }
	return undef;
}

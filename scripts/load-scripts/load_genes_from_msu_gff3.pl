...
load_genes_from_msu_gff3.pl - load grape genes into EnsemblDB
...
=head1 DESCRIPTION
  ...
  Parses the specified MSU gff3 file from MSU RICE project (seperated from TIGR), 
...
foreach my $gene_id( keys %$GENES ){
  ...
  foreach my $dbentry( make_dbentries($gene_xref) ){
    ...
    if( $dbentry->dbname eq 'MSU_LOCUS' ){ # Use as display_xref
      $eGene->display_xref( $dbentry );
    }
  }
  
  
  foreach my $trpt_id (keys %{$genedata->{TRANSCRIPTS}} ){
    my $trptdata = $genedata->{TRANSCRIPTS}->{$trpt_id};
    ...
    $eTranscript->description( $genedata->{GENE_DESCRIPTION} );
    ...
    foreach my $dbentry( make_dbentries($transcript_xref) ){
      ...
      if( $dbentry->dbname eq 'MSU_LOCUS_MODEL' ){ # Use as display_xref

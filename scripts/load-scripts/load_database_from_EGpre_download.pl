...
if(!defined($root)){
  die "No root given i.e. /scratch/weix/Grm37/EG18/ and no database name given to try and guess root from";
}

chdir $root or die "cannot chdir $root\n";
print "Succeded in chdir $root\n";
#my $com = "cd $root"; #print "$com\n";
#my $line= system ("echo $root");
#if($? or $line =~ /Error/ or $line =~ /ERROR/){
#  print $line;
#  die "Error\n";
#}
#else{
#  print "Succeded in doing $com \n" unless $quiet;
#}
...
while (<CHK>){
  ...
  my $str= "mysqlimport --fields_escaped_by=\\\\ --local $mysql_options ".$ENV{"PWD"}."/$database/$table$index.txt";
  print "$str\n" unless $quiet;
  $line = `$str`;
  if($line =~ /Error/ or $?){
    print STDERR $line;

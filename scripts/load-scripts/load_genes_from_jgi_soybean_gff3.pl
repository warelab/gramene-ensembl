...
#print "\n\n[DEBUG] get_gene_id $gene_id, feature=$feature xxxx\n\n";
  if( $feature =~ $GENE_REGEX){
    ...
    $GENES->{$gene_id}->{GENE_NAME}  = $name || $id;
  ...
  if( $feature eq 'MRNA' ){
    ...
    $GENES->{$gene_id}->{TRPT}->{$id}->{TRPT_NAME}  = $name || $id; 
  ...
  if( $feature =~ $CDS_REGEX ){
    ...
    $start += $phase if $strand eq '+';
    $end -= $phase if $strand eq '-';
    if($start <= $end ){

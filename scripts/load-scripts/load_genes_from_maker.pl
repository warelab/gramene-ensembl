...
Readonly my $ATTRIBS => qr/(NOTE|EVIDENCE|FLCDNA)/i;
...
while( my $line = $GFF_HANDLE->getline ){
  ...
  my $id = $attribs{ID} ;
  my @parents = defined $attribs{PARENT} ? split /,/, $attribs{PARENT} : undef;
  $PARENT{ $id } = [@parents] if defined $parents[0];
  if( $feature eq 'MRNA' && !$PARENT{ $id }->[0] ){
      my @p = split '-', $id;
      $p[0] =~ s/T/G/;
      @parents = ($p[0]);
  ...
  #my @gene_ids = $parents[0] ? &get_gene_ids( \@parents ) : ($id);
  my $gene_ids = [];
  if($feature =~ $GENE_REGEX){
      $gene_ids = [$id];
  }else{
      
      &get_gene_ids( \@parents, $gene_ids );
  }
  
  foreach my $gene_id( uniq @$gene_ids){
 

print "\n\n[DEBUG] get_gene_id $gene_id, feature=$feature xxxx\n\n";
  if( $feature =~ $GENE_REGEX){
    print "[INFO] Processing gene $gene_id, seqname = $seqname\n" if $V;
  ...
  if( $feature eq 'MRNA' ){
    ...
    $GENES->{$gene_id}->{TRPT}->{$id}->{DESCRIPTION} = $description;
  }
  ...
  if( $feature eq 'EXON' 
	#||
      ...
      foreach my $parent (uniq @parents){
	  print "EXON_COUNT increment for $gene_id -> $parent\n" if $V;
	  ...
	  if( defined $GENES->{$gene_id}->{TRPT}->{$parent}->{EXON}{$start}){
	      my $pre_exon_end = $GENES->{$gene_id}->{TRPT}->{$parent}->{EXON}{$start};
	      warn("Trpt $parent: Identical exon start $start, pre-exsinting one is $start -> $pre_exon_end, this one is $start => $end, skip it\n");
	      
	      $GENES->{$gene_id}->{TRPT}->{$parent}->{SKIP}=1;
	      next;
  ...
  if( $feature =~ $CDS_REGEX ){
	  ...
	  if (defined $GENES->{$gene_id}->{TRPT}->{$parent}->{CDS_EXON}{$start}){
	      my $pre_cd_end = $GENES->{$gene_id}->{TRPT}->{$parent}->{CDS_EXON}{$start};
	      warn ("Trpt $parent: Identical cd start $start, pre-exsinting one is $start -> $pre_cd_end, this one is $start => $end, skip it\n");
	      $GENES->{$gene_id}->{TRPT}->{$parent}->{SKIP}=1;
	      next;
	  }else{
...
#	print "$trptid => skip value is $genedata->{TRPT}->{$trptid}->{SKIP}\n";
	if  ($genedata->{TRPT}->{$trptid}->{SKIP}){
	...
	print "trptid=$trptid, exon cnt=$trptdata->{EXON_COUNT}\n" if $V;
	#print "EXON key cnt is ", scalar keys %{$trptdata->{EXON}}, "\n" if $V;
	# Get exons into gene-order
	my $f = $genedata->{STRAND} > 0 ? 1 : 0;

	unless ( defined $trptdata->{CDS_EXON} ){
	    print "No CDS_EXON defined" if $V;
...
#EXON_START EXON_END CDS_EXON_START CDS_EXON_END
  ...
  # Find the translation start/stop
    ...
    print "start_codon=$start_codon, stop_codon=$stop_codon\n\n";
    if( $genedata->{STRAND} < 0 ){ 
      ($start_codon,$stop_codon) = ($stop_codon,$start_codon);
    }
    unless( $trptdata->{START_CODON} ){
      warn( "[WARN] Gene $geneid has no predefined start_codon\n" ) if $V;
    }
    unless( $trptdata->{STOP_CODON} ){
      warn( "[WARN] Gene $geneid has no predefined stop_codon\n" ) if $V;
    }
	...
	( $trptdata->{STOP_CODON}  and
      ...
      warn( "[WARN]Start/stop for gene $geneid inconsistent with CDS_EXONS\n" ) if $V;
    }
    $trptdata->{START_CODON} = $start_codon;
    $trptdata->{STOP_CODON}  = $stop_codon;

	#print "number of exon_start is ", scalar @{$trptdata->{EXON_START}}, "\n" if $V;
	...
	print "last_start=$last_start, last_end=$last_end, start=$start, end=$end\n" if $V;
	if( $last_end >= $start and $last_start <= $end ){
	  warn( "[WARN] Transcript $trptid  has overlapping exons. Dubious. Skip it.\n" );
	  delete( $genedata->{TRPT}->{$trptid} );
	  next;
	}
      }
    ...
    unless( defined $trptdata->{CDS_END_EXON} &&
      ...
      warn( "[*DIE] Transcript $trptid has no CDS_END_EXON/CDS_START_EXON, skip it\n" );
      delete( $genedata->{TRPT}->{$trptid} );
      next;
    }
  }

    if (scalar keys %{$GENES->{$geneid}->{TRPT}} < 1){
	delete( $GENES->{$geneid} ) ;
	warn("Gene $geneid has no transcript, skip it\n");
    }
}
...
foreach my $geneid( sort keys %$GENES ){
  ...
  warn ("geneid=$geneid, seq_name=$genedata->{SEQ_NAME}\n");
...
print "\n\nDEBUG  geneid=$geneid\n";
  ...
  $eGene->description( $genedata->{DESCRIPTION} || '');
  $eGene->version(1);  
...
print "trpid=$trptid\n";
    ...
    $eTranscript->description( $trptdata->{DESCRIPTION} || '');
    ...
    my $transcript_xref = [{
			    ...
			    description   => $trptdata->{DESCRIPTION}  || '',
			   },
			  ];
...
print "exon count is $trptdata->{EXON_COUNT}\n";
	...
	print "exon start=$exon_start, exon end=$exon_end\n";
      $eExon->end($exon_end);
...
sub get_gene_ids{
  ...
  foreach my $id(uniq @$ids){
      ...
      #my @genes = ($id);
      if( defined $PARENT{ $id }->[0]){
	  &get_gene_ids( $PARENT{ $id}, $gene_list);
      }else{   
	  print "return ID $id\n"; 
	  push @$gene_list, $id;
	  #return;

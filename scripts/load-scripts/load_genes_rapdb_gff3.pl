...
while( my $line = $GFF_HANDLE->getline ){
  ...
  if( $feature eq 'GENE' ){
    ...
    $gene_description = $attribs{DESCRIPTION} || $attribs{NOTE}  || $gene_name;
    ...
    $gene_id         = $attribs{PARENT} || $attribs{LOCUS_ID};

    next if ( $GENES->{$gene_id}->{TRANSCRIPTS}->{$transcript_id} );

    $attribs{DESCRIPTION} = $attribs{NOTE}  || $transcript_name;
...
foreach my $g( keys %$GENES ){
  ...
  foreach my $t( keys %{$genedata->{TRANSCRIPTS}} ){
    ...
    if( $NONCODING && $genedata->{TRANSCRIPTS}->{$t}{DESCRIPTION} !~ /^Non-protein coding/ ){
   	delete $genedata->{TRANSCRIPTS}->{$t};
	next;	 
    }
...
warn ("start codon coord=$start_codon, stop codon coord =$stop_codon\n");
    ...
    unless( $NONCODING ||  (defined $trptdata->{CDS_END_EXON} &&
      ...
      warn( "Gene-Transcript ${g}-${t} has no CDS_END_EXON/CDS_START_EXON" ) unless ($trptdata->{DESCRIPTION} =~ /^Non-protein coding/);
    }
...
# teasing out nonProtein coding genes
  if( $agenedata->{DESCRIPTION} =~ /^Non-protein coding/i){
	warn ("$gene_id\tNon-protein coding\n");
	#next;
  }
  ...
  my $transcrupt_cnt = scalar (keys %{$agenedata->{TRANSCRIPTS}});
  warn ("number of trancript for ", $agenedata->{GENE_NAME}, "is $transcrupt_cnt\n"); 
  next unless keys %{$agenedata->{TRANSCRIPTS}} > 0;
 
  foreach my $trpt_id (keys %{$agenedata->{TRANSCRIPTS}} ){
    my $atrptdata = $agenedata->{TRANSCRIPTS}->{$trpt_id};
    my $trpt_name =  $atrptdata->{TRPT_NAME};

   if( ! $NONCODING && $atrptdata->{DESCRIPTION} =~ /^Non-protein coding/i){
        warn ("$gene_id\t$trpt_id\tNon-protein coding\n");
        next;
   }
...
#warn ("Show eTranscript: ", Dumper $eTranscript); 

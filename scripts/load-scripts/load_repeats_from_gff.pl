...
while( my $line = $GFF_HANDLE->getline ){
  ...
  if ( $feature eq 'MASK_REGION' || $feature eq 'TRANSPOSON'){    
  
     
      if( $attribs{TARGET} ){
	  $source =  $attribs{TARGET};
      }
     
      if( $attribs{CLASS} ){
	  $rc_name =  $attribs{CLASS};
      }      
      
      my $rc = Bio::EnsEMBL::RepeatConsensus->new
	  (
	   -NAME => $source,
	   -REPEAT_CLASS => $rc_name, 
	   -ADAPTOR => $rca,
	   );
    ...
    $count++;
    print "stored $count repeat features: $start, $end, $attribs{ID}\n";
  }
  else{

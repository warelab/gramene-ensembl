package ExportView::GFF3ExporterMod;
...
sub initialize {
	...
	for my $k(keys %$self){ warn("DEBUG: $k => $self->{$k}\n"); };	
	return $self;
}
...
sub export_genes_from_slices {
	...
	while (@slices) {
        ...
        my $gene_counter = 0;
        my $num_genes = scalar( $self->{gene_type} ? @{$slice->get_all_Genes_by_type($self->{gene_type})} : @{$slice->get_all_Genes});
...
sub export_genes_from_slice {
	...
	my @Genes = $self->{gene_type} ? @{$slice->get_all_Genes_by_type($self->{gene_type})}:
					@{$slice->get_all_Genes};
	...
	foreach my $gene ( @Genes ) {
		...
		my $num_transcripts = scalar($self->{canonical_transcript} ? $gene->canonical_transcript: @{$gene->get_all_Transcripts});
		...
		$self->export_transcripts(
			...
			$gene,
			$gene_id,
			$self->{canonical_transcript} ? [$gene->canonical_transcript]: $gene->get_all_Transcripts,

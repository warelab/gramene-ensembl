...
fix-archived-clones.pl - Resets all accession version numbers

=head1 SYNOPSIS

perl fix-archived-clones.pl [options]
...
MAIN: {
    ...
    my $clones = get_all_clones($db_adaptor);

    $logger->info("Got @{[ scalar @$clones ]} clones");
    my %current_accessions = ();

    $logger->info("Fixing clone and contig names");
    for my $clone (@$clones) {
        my ($accession, $suff_version)
            = $clone->seq_region_name =~ m/^([A-Z]{2}\d{6})(?:\.(\d+))?$/;
        my $version = version_for($clone);

        if (!defined $accession) {
            $logger->warn(
                "Something's wrong with Clone @{[$clone->seq_region_name]} (ID=@{[$clone->dbID]})"
            );
            next;
        }
        if (not defined $version or $version !~ m/^\d+$/) {
            $logger->warn("Version attribute is not set");
            next;
        }
        my $versioned_accession = "$accession.$version";
        $logger->debug("Clone: ", $clone->seq_region_name,
            " -> $versioned_accession");
        update_slice_name($db_adaptor, $clone, $versioned_accession);

        if (defined $current_accessions{$accession}) {
            if ($version > version_for($current_accessions{$accession})) {
                $current_accessions{$accession} = $clone;
            }
        } else {
            $current_accessions{$accession} = $clone;
        }

        for my $contig (map { $_->to_Slice } @{ $clone->project('contig') }) {
            my ($contig_num) = $contig->seq_region_name =~ m/(Contig\d+)$/;

            my $contig_name = "$versioned_accession-$contig_num";
            $logger->debug("   Contig: ", $contig->seq_region_name,
                " -> $contig_name");
            update_slice_name($db_adaptor, $contig, $contig_name);
        }
    }

    $logger->info("Setting current clones");
    my $attribute_adaptor = $db_adaptor->get_AttributeAdaptor();
    my $current_attribute = Bio::EnsEMBL::Attribute->new(
        -CODE        => 'current-version',
        -NAME        => 'Current Accession Version',
        -DESCRIPTION => 'Identifies the most recent version of an accession',
        -VALUE       => 'current'
    );
    for my $accession (sort keys %current_accessions) {
        my $clone = $current_accessions{$accession};
        $logger->debug("$accession (clone=$clone)");
        $logger->debug("$accession -> ", version_for($clone));
        $attribute_adaptor->store_on_Slice($clone, [$current_attribute])
            unless scalar @{ $clone->get_all_Attributes('current-version') };
    }

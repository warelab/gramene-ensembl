...
sub ADD_ALL_MARKER_FEATURES {
    ...
    for my $track_name (@ordered_tracks) {

        next if $track_name =~ /marker_marker$/i;
        my $caption = $captions{$track_name}
            || $track_name;
        my $colour = 'black';
        my $type   = $track_name;    #'Marker';
        my %extra  = ();

        my $menu = 'markers';
        if ($track_name =~ /tos17/i) { $menu = 'fst_features' }

        # warn "$track_name,$caption,$type,$menu";

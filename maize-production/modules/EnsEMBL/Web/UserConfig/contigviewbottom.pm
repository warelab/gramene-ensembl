...
## Matches ##
            ...
            'menus' =>
                [qw( features options repeats export jumpto resize )]
        },
...
=pod
    Author:  Shiran
    Purpose: Set track labels based on menu item labels (Don't Repeat Yourself)
=cut
...
sub set_mdr_labels {
    ...
    for my $mdr_setting (@{ $settings->{'mdr_features'} }) {
        my $mdr_track = $contigviewbottom{ $mdr_setting->[0] };
        $mdr_track->{'track_label'} = $mdr_setting->[1];
    }
}

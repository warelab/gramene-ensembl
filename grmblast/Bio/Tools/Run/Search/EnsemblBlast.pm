...
#command is /usr/local/wublast/blastn /usr/local/blastdb/gramene/Oryza_sativa.IRGSP-1.0.dna.toplevel.fa [[]]  -N -3 -M 1 -B 100 -E 10 -R 3 -Q 3 -filter dust -W 15
...
#PROJECT-NAME: blastn:Oryza_sativa.IRGSP-1.0.dna.toplevel.fa:1260:dna
   
 #  my $command_line = qq(|bsub -c 120 -q $queue -P '$project_name' -J $ticket -o /dev/null -f "$client_fasta_file > $server_fasta_file");
 #  warn $command_line;
#|bsub -c 120 -q offline -P 'blastn:Oryza_sativa.IRGSP-1.0.dna.toplevel.fa:1260:dna' -J susXGCWa -o /dev/null -f "/usr/local/ensembl-grmblast/tmp/blastqueue/BLA_gO/W3ApqD6/susXGCWa.fasta > /tmp/susXGCWa.fa"
...
#   if( open(BSUB, $command_line )) {
   ...
   $self->_add_command(
		       qq( cp $client_fasta_file $server_fasta_file )
		       );
      ...
      $self->_add_command(
        qq($command >$server_out_file 2>$server_fail_file),       ## Run the blast, sending output to local temp file
         #q(export status=$?),                                             ## Store status of BLAST command
	 #q(echo "status=$status"),
        qq(echo '$state_file' > $server_flag_file),               ## Touch flag file so that can indicate blast has finished
        qq(cp "$server_out_file"  "$client_out_file"  ),
        qq(cp "$server_fail_file" "$client_fail_file" ),
        qq(cp "$server_flag_file" "$client_flag_file" ), # Copy all files back...
        qq(rm -f /tmp/$ticket.*),                                 # Now tidy up the temporary files
         #q(exit $status)                                          # Return exit codo of $command!
      );
warn $self->_command_string();
      system ($self->_command_string());
      #close BSUB;
   #my $status = $ENV{status};
   #warn ("status is $status\n");
   if ($? != 0) {
       #the returned status is going to be always 0, we didn't actually check
       # the failure or success of the blast
        die("modified bsub exited with non-zero status - job not submitted\n");
      }
#    } else {

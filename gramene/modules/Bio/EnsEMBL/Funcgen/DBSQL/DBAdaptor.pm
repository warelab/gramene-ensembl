...
sub new {
  ...
  $self->{'dnadb_host'} = $dnadb_host || $default_host || 'cabot.cshl.edu'; #'ensembldb.ensembl.org';
  $self->{'dnadb_port'} = $dnadb_port || $default_port || 3306;
  $self->{'dnadb_user'} = $dnadb_user || $default_user || 'gramene_web'; #'anonymous';
  $self->{'dnadb_pass'} = $dnadb_pass || $default_pass || 'gram3n3'; #undef;
  $self->{'dnadb_name'} = $dnadb_name || $default_name || 'sorghum_bicolor_core_1_87_30';

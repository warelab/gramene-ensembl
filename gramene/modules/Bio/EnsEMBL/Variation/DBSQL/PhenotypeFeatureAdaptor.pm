...
sub _is_class_constraint {
  ...
  # get attribute
  my $pheno_constraint = $classes ? qq{ p.class_attrib_id in ($classes) } : '';
  $constraint  .= (defined($constraint)) && $pheno_constraint ? " AND$pheno_constraint" : $pheno_constraint;
...
#warn("DEBUG get_all_phenotype_classes: inside if ", @{$self->{phenotype_classes}}, "\n");
  }

#warn("DEBUG get_all_phenotype_classes: after if", @{$self->{phenotype_classes}}, "\n");
...
sub use_phenotype_classes {
  ...
  if ( !defined($self->{use_phenotype_classes}) && !defined($include)) {
    ...
    @classes = defined $self->get_all_phenotype_classes() ? 
	keys %{$self->get_all_phenotype_classes()} : ();

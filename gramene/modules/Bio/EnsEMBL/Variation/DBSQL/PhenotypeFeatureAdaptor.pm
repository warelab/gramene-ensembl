...
sub _is_class_constraint {
  ...
  # get attribute
  my $pheno_constraint = qq{ p.class_attrib_id in ($classes) } if $classes;
#warn("DEBUG weix classes is $classes, pheno_constraint=$pheno_constraint\n");
  $constraint  .= (defined($constraint)) ?  defined ($pheno_constraint) ? 
					" AND$pheno_constraint" : ''
				: $pheno_constraint;
...
sub use_phenotype_classes {
  ...
  if ( !defined($self->{use_phenotype_classes}) && !defined($include)) {
	...
	my $r = $self->get_all_phenotype_classes();
    @classes = keys %{$r} if $r;

...
sub content {
  ...
  if ($self->hub->action && $self->hub->action eq 'ExpressionAtlas' && $self->hub->gxa_status) {
    ...
    $main_js .=  qq{
      <script language="JavaScript" type="text/javascript" src="https://dev.gramene.org/static/vendorCommons.bundle.js"></script>
      <script language="JavaScript" type="text/javascript" src="https://dev.gramene.org/static/expressionAtlasHeatmapHighcharts.bundle.js"></script>
    };
  }
  ...
  if ($self->hub->action && $self->hub->action eq 'Pathway' && $self->hub->pathway_status) {
	...
	my $js_file = ($SiteDefs::SUBDOMAIN_DIR =~ /plant/i) ?   

...
sub _get_dom_tree {
  ...
  if (scalar @ok_species > 1) {
    ...
    my $homepage      = $hub->url({'species' => $species, 'type' => 'Info', 'function' => 'Annotation', '__clear' => 1});
...
sub species_list_url { return '/species.html'; }
...
sub _species_list {
  ...
  for (@fav, sort {$species->{$a}{'display_name'} cmp $species->{$b}{'display_name'}} keys %$species) {
    ...
    my $homepage      = $hub->url({'species' => $_, 'type' => 'Info', 'function' => 'Annotation', '__clear' => 1});

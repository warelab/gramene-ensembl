...
memoize('logic_name_label');
memoize('grouping_menu');
memoize('grouping');

Readonly my @BIOTYPE_COLORS => (
    'KNOWN'         => [ 'rust',       'Known Gene' ],
    'NOVEL'         => [ 'mediumblue', 'Novel Gene' ],
    'PSEUDOGENE'    => [ 'darkgrey',   'Pseudogene' ],
    'KNOWN_HOMOLOG' => [ 'maroon',     'Known Gene, has Ortholog' ],
    'NOVEL_HOMOLOG' => [ 'mediumblue', 'Novel Gene, has Ortholog' ],
    'TRANSPOSON_PSEUDOGENE' =>
        [ 'maroon', 'Similarity to transposable elements' ],
    'PROTEIN_CODING' => [ 'mediumblue', 'Similarity to known proteins' ],
    'PROTEIN_CODING_UNSUPPORTED' =>
        [ 'darkgrey', 'No similarity to known proteins' ],
...
=head2 add_legend
    ...
    Add a legend for a given track
...
sub add_legend {
    my $self         = shift;
    my (%parameters) = @_;
    my $track        = $parameters{'track'};
    my $code         = $parameters{'name'} || "${track}_legend";

    my $caption = $parameters{'caption'};
    our $legend_position ||= 20000;

    my $script   = $self->script();
    my $artefact = $self->{'general'}{$script}{$track};
...
sub get_available_artefacts {
    ...
    foreach ($self->subsections()) {
        ...
        # Test availability
        push(@available_artefacts, $_)
            if $self->is_available_artefact($_);
    }
...
sub values {
    ...
    return {}
        unless (defined $self->{'general'}->{$script}->{$subsection});
...
sub set {
    ...
    my $script = $self->script();
    return
        unless (defined $key && defined $script && defined $subsection);
    if ($force == 1) {
        $self->{'user'}->{$script}->{$subsection} ||= {};
    } else {
        return unless (defined $self->{'general'}->{$script});
...
sub ADD_ALL_DNA_FEATURES {
    ...
    for my $menu_type (qw/gss est array fst/) {
        $menu_types{$menu_type} = join "|",
            @{ $self->grouping_menu($menu_type) };
    }
    ...
    # Note on the sort -
    # Push Maize and Swissprot to the top,
...
sub ADD_ALL_PROTEIN_FEATURES {
    ...
    my %captions = (
        ...
        'BLASTPEPTIDES' => 'Protein Alignments',
    );
    my %colours = (
        'BLASTPEPTIDES' => 'darkorange',
        'SWISSPROT'   => 'green',
        'RICE'        => 'green',
        'MAIZE'       => 'red',
        'WHEAT'       => 'blue',
    ...
    {
        my ($species, $type) = split('_', uc($logic), 2);
        my $colour  = $colours{ uc($logic) }      || 'black';
...
sub ADD_AS_GENE_TRACKS {
    ...
    $self->add_new_track_gene(
        ...
        'gene_label' => sub {
            return
                $_[0]->biotype eq 'bacterial_contaminant' ? 'Bac. cont.'
                : (
        ...
        'gene_col' => sub {
            return
                $_[0]->biotype eq 'bacterial_contaminant' ? '_BACCOM'
                : (
...
sub logic_name_label {
    ...
    my %labels = (
        ...
        'MAIZE_CDS_PANZEA' => 'Maize PanZea CDSs',
        ...
        'MAIZE_FOSMIDEND'                => 'Maize Fosmid Ends',
        ...
        # Array tracks
        'WHEAT_CONSENSUS'             => 'Wheat Affy A61K',
        'RICE_CONSENSUS_AFF'          => 'Rice Affy A57K',
        'RICE_ARRAYCONSENSUS_AFFY57K' => 'Rice Affy A57K',
        'MAIZE_CONSENSUS_AFF'         => 'Maize Affy A18K',
        'WHEAT_CONSENSUS_AFF'         => 'Wheat Affy A61K',
        'BARLEY1_GENECHIP_EXEMPLARS'  => 'Barley Affy GeneChip 22K',
        'RICE_ESTOLIGO_TIGR'          => 'Rice Oligo NSF 20K',
        'RICE_ARRAYOLIGO_YALE58K'     => 'Rice Oligo Yale 58K',
        ...
        'CYTO_DNA_ALIGNMENTS' => 'DNA Alignments',
        ...
        'MAIZE_CONSENSUS'                 => 'Maize Cornsensus',
        'MAIZE_ESTCLUSTER_PLANTGDB'       => 'Maize PlantGDB ESTCl.',
        'MAIZE_MAGI_ISU'                  => 'Maize ISU MAGI',
        'MAIZE_ISU_MAGIS_3_1_W_SINGLETON' => 'Maize ISU MAGI (Singleton)',
        'MAIZE_MARKERS'                   => 'Maize FPC Markers',
        'MAIZE_MRNA'                      => 'Maize mRNA',
        'MAIZE_SEQ_PANZEA'                => 'Maize PanZea Sequences',
...
=head2 grouping_menu
    ...
    Returns a list of type features formatted for display in menu
...
sub grouping_menu {
    ...
    my @menu_items = reverse sort uniq map { uc($_) =~ m/^[A-Z]+_(.*)$/ }
        @{ $self->grouping($type) };
    return \@menu_items;
}
...
=head2 grouping
    ...
    Returns a grouping of features by type
...
sub grouping {
    ...
    return {
        'fst' => [
            qw(Rice_FstTransposon Rice_T_DNA_Insert Rice_tos17_insert
                Maize_Mu_Insert)
        ],
        'gss' => [
            qw(RiceAlta_BACend_OMAP RiceAustraliensis_BACend_OMAP
                RiceBrachyantha_BACend_OMAP RiceCoarctata_BACend_OMAP
                RiceGlaberrima_BACend_OMAP RiceGranulata_BACend_OMAP
                RiceJaponica_BACend_OMAP RiceMinuta_BACend_OMAP
                RiceNivara_BACend_OMAP RiceOfficinalis_BACend_OMAP
                RicePunctata_BACend_OMAP RiceRidleyi_BACend_OMAP
                RiceRufipogon_BACend_OMAP Maize_meth_filt_CSHL_Mccombie
                Maize_meth_filt_hi_cot_cluster Maize_meth_filt_TIGR
                Maize_hi_cot_Bennetzen Maize_hi_cot_TIGR Ryegrass_Assembly
                Ryegrass_Sequence Sorghum_gss-read_Klein Sorghum_orion
                Maize_BACend Maize_FosmidEnd Maize_Seq_Panzea)
        ],
        'est' => [
            qw(Barley_est Barley_ESTCluster_PlantGDB Barley_GI Maize_est
                Maize_ESTCluster_PlantGDB Other-poaceae_est Rice_est
                Rice_ESTCluster_PlantGDB Sorghum_est Sorghum_ESTCluster3P_LGBPratt
                Sorghum_ESTCluster_PlantGDB Wheat_est Wheat_ESTCluster_PlantGDB
                Rice_mRNA Maize_GI Rice_GI Sorghum_GI Wheat_GI Sugarcane_est
                Rice_ind_cluster Rice_ind_est Rice_jap_cDNA_KOME Rice_CDS
                Maize_consensus Sorghum_CDNA)
        ],
        'array' => [
            qw(Wheat_ArrayConsensus_Affy61K Rice_ArrayConsensus_Affy57K
                Rice_ArrayOligo_Yale58K Maize_ArrayConsensus_Affy18K
                Maize_ArrayTarget_NSF58K Barley_ArrayConsensus_Affy22K)
        ],
    }->{ lc $type };
}
...
=head2 biotype_label
    ...
    Returns a descriptive label for a given biotype
...
sub biotype_label {
    ...
    my ($biotype) = @_;
    return +{
        'protein_coding'             => 'Similar to known genes',
        'protein_coding_unsupported' => 'Hypothetical',
        'transposon_pseudogene'      => 'Transposon-like',
    }->{ lc $biotype };
}

...
=head2 fetch_sequenced_bacs_for_slice

    Returns a list of BACs for a given FPC slice sorted by coordinate

=cut

sub fetch_sequenced_bacs_for_slice {
    my $self  = shift;
    my $slice = shift;
    die "Undefined slice" unless defined $slice;

    assert_class('Bio::EnsEMBL::Slice', $slice);

    my @sorted = sort { $a->seq_region_start <=> $b->seq_region_start }
        @{ $slice->get_all_MiscFeatures('acc_bac_map') };
    return \@sorted;
}

=head2 bac_dna_features

    Returns DNA alignments for a given BAC

=cut

sub bac_dna_features {
    my $self = shift;
    my ($bac_feature, $feature_method, $subtypes, $argument_index) = @_;

    assert_class('Bio::EnsEMBL::MiscFeature', $bac_feature);

    my $accession
        = $bac_feature->get_scalar_attribute(FEATURE_ATTRIBUTE_ACCESSION);
    my $bac_adaptor = $self->_slice_adaptor(BAC_SPECIES);
    my $bac = $bac_adaptor->fetch_by_region('clone', $accession);
    return undef unless defined $bac;

    if (!$bac->can($feature_method)) {
        throw("Unable to call method <$feature_method> on BAC Slice $bac");
    }

    my $features = {};
    if (scalar @$subtypes == 0) {
        my $num_features = scalar @{ $bac->$feature_method() };
        $features->{'All features'} = $num_features;
    } else {
        my $all_features = 0;
        for my $subtype (@$subtypes) {
            my @arguments = ($subtype);
            for (my $i = 0; $i < $argument_index; $i++) {
                unshift @arguments, undef;
            }
            my $num_features
                = scalar @{ $bac->$feature_method(@arguments) };
            next if $num_features == 0;

            $features->{$subtype} = $num_features;
            $all_features += $num_features;
        }
        if ($all_features > 0) {
            $features->{'All features'} = $all_features;
        }
    }
    return $features;
}
...
SQL
    ...
    $statement->execute();
    my $latest = $statement->fetchrow_arrayref()->[0];
    return $latest;
}
...
=head2 assert_class
    ...
    Asserts that a given object is of a given type.
...
sub assert_class {
    ...
    die "Argument '$object' is not a $type"
        unless ref($object) eq $type;

...
sub context_menu {
    ...
    my $coordinates = $obj->thousandify(floor($obj->seq_region_start));
    if (floor($obj->seq_region_start) != ceil($obj->seq_region_end)) {
        $coordinates .= " - " . $obj->thousandify(ceil($obj->seq_region_end));
    }
    my $header = <<HTML;
@{[$obj->seq_region_type_and_name]}<br />
<span id="nav-coordinates">
$coordinates
</span>
HTML

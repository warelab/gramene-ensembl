...
use SpeciesDefs;
use ExtIndex;
use EnsMart::BlastDefs;

use EnsMart::Meta;
use EnsMart::MetaStage;
use EnsMart::MetaBlock;
use EnsMart::MetaForm; 
use EnsMart::MetaFormEntry;
use EnsMart::MetaHyperlink;

use Bio::Tools::Run::Search;
#use blast_parser::Util qw( get_unique_id );
#use EnsMart::MetaInstance;
#use vars qw( $GLOBAL $DEFS $SPECIES_DEFS );
our $GLOBAL       = EnsMart::Meta->new;
our $DEFS         = EnsMart::BlastDefs->new;
our $SPECIES_DEFS = SpeciesDefs->new;
...
STAGE_SETUP:{
 ...
 BLOCK_SEQUENCE:{
  ...
  FORM_SEQUENCE:{
	...
	$entry->set_label
	  ( "<B>Either</B> Paste sequences (max 10) in FASTA or plain text:" );
...
#	    } );
 ...
 BLOCK_ABOUT:{
    my $block = $stage->addobj_block();
    $block->set_available(0); # Suppress
  ...
  FORM_ABOUT:{
	...
	$entry->set_label( qq(
<SMALL>BlastView provides an integrated platform for sequence similarity searches against $sitetype databases, offering access to both BLAST and SSAHA programs. <A href='/Homo_sapiens/helpview?se=1&kw=blastview' target='onlinehelp'>[More]</A><BR/><IMG src="/gfx/blank.gif" height=5><BR>
...
#	$entry->set_label_summary('Ticket ID: %s');
 ...
 BLOCK_USAGE:{
  ...
  FORM_USAGE:{
	...
	my $entry = $form->addobj_form_entry();
	$entry->set_label( qq( 
<SMALL>'Job pending' results can be retrieved by clicking on the button above. Alternatively, this page can be bookmarked for later, or the ID noted and entered on the BLAST page.<br/><IMG src="/gfx/blank.gif" height=5><br/>
...
function setSensitivity(){
  ...
  for( var i=0; i<sensitivityValues.length; i++ ){
    ...
    if( sensitivityConf[me][sens] ){
      sensValues.push( sens );
    }
  }
...
// Disable for now
//  for( var i=0; i<speciesAry.length; i++ ){
//    var species = speciesAry[i];
//    if( typeof getConf( queryType, species ) == 'object' ){
//      enableRadio( control, species );
//    }
//    else{
//      disableRadio( control, species );
//    }
//  }
}
...
function setDatabaseType(){
  ...
  for( var i=0; i<disableAry.length; i++ ){
    disableRadio( radio, disableAry[i] );
  } 
}
...
function setDatabase(){
  ...
  for( var i=0; i<selSpecies.length; i++ ){
    ...
    for( var j=0; j<databaseAry.length; j++ ){
      ...
      if( typeof getConf( selQType, sp, 'dna', db ) == 'object' ){
        nAry.push( db );
      }
      if( typeof getConf( selQType, sp, 'peptide', db ) == 'object' ){
        pAry.push( db );
      }
    }
  ...
  //for( var i=0; i<optNuclValues.length; i++ ){
  //  optNuclLabels[optNuclValues[i]] = methodLabels[optNuclValues[i]];
  //}
  //for( var i=0; i<optProtValues.length; i++ ){
  //  optProtLabels[optProtValues[i]] = methodLabels[optProtValues[i]];
  //}

  setSelectOptions( nuclTargetDB, optNuclValues, 
                    dbDnaLabels, lastDatabaseDna );
  setSelectOptions( protTargetDB, optProtValues, 
                    dbPeptideLabels, lastDatabasePeptide );

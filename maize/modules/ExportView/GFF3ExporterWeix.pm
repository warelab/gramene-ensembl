...
sub export_genes_from_slice {
	...
	foreach my $gene ( @{ $slice->get_all_Genes } ) {
		...
		my $transcript_counter = 0;
 		print STDERR "@ gene (", $gene->stable_id, ") @ ", scalar(localtime), "\n" if $self->{'debug'};use Data::Dumper; #XXX debugging;
...
sub export_pairwise_wga_from_slice {
	...
	my $source = $mlss->method_link_type;
	...
	foreach my $gab ( @{ $gabAdaptor->fetch_all_by_MethodLinkSpeciesSet_Slice($mlss, $slice) } ) {
	   ...
	   # my $attribute = 			    (map { $self->escape_attribute() }
		#	     map {"$_=$attributes->{$_};"} qw(ID Name Target) )
		#	     ),
		
	    my $feature_line= join("\t",
			   ((map { $self->escape($_) }
			    (
				$seqid,					#seqid
				$source,					#source
			    ...
			    (join ";", 
			     map { "$_=" . $self->escape_attribute($attributes->{$_}) } qw(ID Name Target) )
			     )
			
				   );
		
	    
	    print $fh "$feature_line\n";
	   # last;
	}# end foreach GAB map { $self->escape_attribute() }

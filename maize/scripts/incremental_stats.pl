...
# NOTE: Before run the program, change the values of maximum seq_region_id for clone and contig
# (1) To run program, issue: perl ./incremental_stats.pl -e ./ensembl.registry -s Zea_mays -f incremental_stats
# (2) To run program, issue: nice nohup perl ./incremental_stats.pl -e ./ensembl.registry -s Zea_mays  -f incremental_stats >& incremental_stats.out &

=pod

=head1 NAME

incremental_stats.pl  - generate incremental stats report from Ensembl database

=head1 SYNOPSIS

perl incremental_stats.pl [options]
...
my $last_clone_max_seq_region_id = 312504; # 263041; 287907; 309520
my ($clone_max_seq_region_id,
    $total_clones,
    $clone_min_size, $clone_max_size,
    $clone_ave_size, $clone_median_size,
    $clone_stddev_size) = &get_clones_stats('clone', $last_clone_max_seq_region_id);

# Get contigs statistics
my $last_contig_max_seq_region_id = 312514; # 263060; 287928; 309543;
my ($contig_max_seq_region_id,
    $total_contigs,
    $contig_min_size, $contig_max_size,
    $contig_ave_size, $contig_median_size,
    $contig_stddev_size) = &get_contigs_stats($last_contig_max_seq_region_id);
...
my ($contigsperclone_min_size, $contigsperclone_max_size,
    ...
    $contigsperclone_stddev_size) = &get_contigs_perclone_stats($last_contig_max_seq_region_id);
...
my ($total_FGENESH_gene_models,
    ...
    $genemodel_stddev_size) = &get_gene_models_stats($last_contig_max_seq_region_id);

#==========================================================================================================================
# NOTE: IF NO GENE MODELS ARE ANALYZED FOR THE WEEKLY NEW CLONES, DON'T CALL THIS 'get_gene_model_classes_stats' SUBROUTINE.
# Since $total_FGENESH_gene_models would be zero.
#========================================================================================================================== 
...
#    ) = &get_gene_model_classes_stats($total_FGENESH_gene_models, $last_contig_max_seq_region_id);


#================================================================================================================================
# NOTE: IF NO GENE MODELS ARE ANALYZED FOR THE WEEKLY NEW CLONES, DON'T CALL THIS 'get_gene_models_nucleotides_stats' SUBROUTINE.
# Since this subroutine would encounter division by zero 
#================================================================================================================================
...
#    ) = &get_gene_models_nucleotides_stats ($last_contig_max_seq_region_id);


# Get total nucleotides of the current BAC clones
my $total_nucleotides = &get_total_nucleotides($last_clone_max_seq_region_id);
...
my ($total_mdr_025_nucleotides, $percent_mdr_025_nucleotides,
    ...
    $total_mdr_3_nucleotides, $percent_mdr_3_nucleotides) = &get_mdr_stats($total_nucleotides, $last_contig_max_seq_region_id);

#=================================================================================================================
# NOTE: IF NO GENE MODELS ARE ANALYZED FOR THE WEEKLY NEW CLONES, DON'T CALL THIS 'get_interpro_stats' SUBROUTINE.
# Since this subroutine would encounter division by zero
#=================================================================================================================
# Get Interpro statistics (NOTE: $interpro is reference to a hash)
#my ($interpro) = &get_interpro_stats($total_FGENESH_gene_models, $last_contig_max_seq_region_id);

# Create QC report table
&create_QC_report_table($clone_max_seq_region_id, $total_clones, $clone_min_size, $clone_max_size, $clone_ave_size, $clone_median_size, $clone_stddev_size,
                        $contig_max_seq_region_id, $total_contigs, $contig_min_size, $contig_max_size, $contig_ave_size, $contig_median_size, $contig_stddev_size,
...
sub get_clones_stats {
   my ($coord_system_name, $last_max_seq_region_id) = @_;

   if (!$coord_system_name) { die( "Need coord system name" ); }
   if (!$last_max_seq_region_id) { die( "Need clone's last maximum seq_region_id " ); }  

   # Get clone coord_system_id
   my $coord_system_id = &get_coord_system_id($coord_system_name);
 
   # Query to get max seq_region_id 
   my $max_seq_region_id_sql = "select max(s.seq_region_id) from ".
                               "seq_region s, seq_region_attrib sa ".
			       "where s.seq_region_id= sa.seq_region_id and s.coord_system_id=".$coord_system_id.
			       " and sa.value='current'";
   ...
   my $current_clones_sql = "(select s.seq_region_id, s.name, s.length AS LENGTH from ".
                            "seq_region s, seq_region_attrib sa ".
                            "where s.seq_region_id= sa.seq_region_id and s.coord_system_id=".$coord_system_id.
                            " and s.seq_region_id > ".$last_max_seq_region_id. " and sa.value='current') tmpTable";
   ...
   $sth = $ENS_DBA->dbc->prepare($max_seq_region_id_sql);
   $sth->execute();
   my $max_seq_region_id = $sth->fetchrow_array();
   ...
   if ($total % 2 == 1) {
      ...
      $median_sql = "select s.seq_region_id, s.name, s.length AS LENGTH from seq_region s, seq_region_attrib sa ".
                    "where s.seq_region_id= sa.seq_region_id and s.coord_system_id=".$coord_system_id.
                    " and s.seq_region_id > ".$last_max_seq_region_id." and sa.value='current' order by s.length limit ".$limit_value;
...
sub get_contigs_stats {
   my ($last_contig_max_seq_region_id) = shift || die( "Need contig's last maximum seq_region_id" );
  
   # Query to get max seq_region_id
   my $max_seq_region_id_sql = "select max(f.seq_region_id) from seq_region a ".
   ...
   my $contigs_sql = "seq_region a left join assembly asm on asm.asm_seq_region_id = a.seq_region_id ".
                     ...
                     "f.coord_system_id = (select coord_system_id from coord_system where name = 'contig') and ".
                     "f.seq_region_id > ".$last_contig_max_seq_region_id." and v.value = 'current'";
   ...
   return ($max_seq_region_id, $total, $min, $max, $ave, $median, $stddev);
}
...
sub get_contigs_perclone_stats {
   ...
   my $contigs_perclone_sql = "(select count(*) AS COUNTING from seq_region a ".
                              ...
                              "(select coord_system_id from coord_system where name = 'clone') and ".
                              "f.coord_system_id = (select coord_system_id from coord_system where name = 'contig') and ".
                              "f.seq_region_id > ".$last_contig_max_seq_region_id." and v.value = 'current' group by asm.asm_seq_region_id) tmpTable";
   ...
   if ($total_entries % 2 == 0) {
      ...
      $median_sql = "select count(*) AS COUNTING from seq_region a ".
                    ...
                    "(select coord_system_id from coord_system where name = 'clone') and ".
                    "f.coord_system_id = (select coord_system_id from coord_system where name = 'contig') and ".
                    "f.seq_region_id > ".$last_contig_max_seq_region_id." and v.value = 'current' group by asm.asm_seq_region_id order by COUNTING limit ".$limit_value;
...
sub get_gene_model_classes_stats {
   my ($total_FGENESH_gene_models, $last_contig_max_seq_region_id) = @_;
   if (!$total_FGENESH_gene_models) { die( "Need total FGENESH gene models" ); }
   if (!$last_contig_max_seq_region_id) { die( "Need contig's last maximum seq_region_id" ); }   
...
sub get_total_nucleotides {
   my ($last_max_seq_region_id) = shift || die( "Need clone's last maximum seq_region_id" );
   ...
   my $total_nucleotides_sql = "select sum(sr.length) from seq_region sr, seq_region_attrib sra ".
                               ...
                               "sr.seq_region_id > ".$last_max_seq_region_id." and sra.value='current'";
   # Get DBConnection. Then use DBConnection's prepare method to prepare sql statement and get DBI statement handle
...
sub get_mdr_stats {
   my ($total_nucleotides, $last_contig_max_seq_region_id) = @_;
   if (!$total_nucleotides) { die( "Need total nucleotides" ); }
   if (!$last_contig_max_seq_region_id) { die( "Need contig's last maximum seq_region_id" ); }   
...
sub create_QC_report_table {
   my ($clone_max_seq_region_id, $total_clones, $clone_min_size, $clone_max_size, $clone_ave_size, $clone_median_size, $clone_stddev_size, 
       $contig_max_seq_region_id, $total_contigs, $contig_min_size, $contig_max_size, $contig_ave_size, $contig_median_size, $contig_stddev_size, 
       $contigsperclone_min_size, $contigsperclone_max_size, $contigsperclone_ave_size, $contigsperclone_median_size, $contigsperclone_stddev_size,
       $total_FGENESH_gene_models, $genemodel_min_size, $genemodel_max_size, $genemodel_ave_size, $genemodel_median_size, $genemodel_stddev_size, 
...
#       $interpro
   ...
   print(TXTOUT "BAC Clone Maximum seq_region_id: ".$clone_max_seq_region_id."\n");
   ...
   print(TXTOUT "Contig Maximum seq_region_id: ".$contig_max_seq_region_id."\n");
...
#   }
   ...
   print(HTMLOUT "<tr>"."\n");
   print(HTMLOUT "<td>BAC Clones (<i>Maximum seq_region_id=".$clone_max_seq_region_id."</i>)<br>"."\n");
   ...
   print(HTMLOUT "<tr>"."\n");
   print(HTMLOUT "<td>Contigs (<i>Maximum seq_region_id=".$contig_max_seq_region_id."</i>)<br>"."\n");
   print(HTMLOUT "Minimum Size (bp)<br>"."\n");
   print(HTMLOUT "Maximum Size (bp)<br>"."\n");
   print(HTMLOUT "Average Size (bp)<br>"."\n");
   print(HTMLOUT "Median Size (bp)<br>"."\n");
